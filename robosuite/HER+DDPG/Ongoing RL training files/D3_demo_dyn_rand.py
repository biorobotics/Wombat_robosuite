...
def _preproc_inputs(obs, g):
    ...
    return inputs
def dyn_rand():
	phone_x = 0.4655#np.random.uniform(0.428, 0.728)
	phone_speed = -0.14#np.random.uniform(-0.14, -0.18)
	phone_orient = 0.0
...
if __name__ == '__main__':
    ...
    model_path = args.save_dir + args.env_name + '/Model_dynamics_epoch_1.pt'
    ...
    for i in range(args.demo_length):
        ...
        for t in range(env._max_episode_steps):
            ...
            elif t>=1600 and t<1700 and pp==0:
                action_network = np.zeros(3)
                obs,reward,done,_ = env.step(action_zero) 
                obs_current = obs['observation']
