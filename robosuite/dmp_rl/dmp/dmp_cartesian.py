...
class DMPs_cartesian(object):
	...
	Paper Link : https://arxiv.org/pdf/2007.00518.pdf
	'''

	def __init__(self,
		n_dmps = 3, n_bfs = 50, dt = 0.01, x_0 = None, x_goal = None, T = 1.0,
		K = 1050, D = None, w = None, tol = 0.01, alpha_s = 4.0, basis = 'gaussian',rescale=None, **kwargs):
		'''
		...
		# D is the damping of the system, for critically damped 
	...
	def step(self, tau = 1.0, error = 0.0, external_force = None,
		...
		def beta_s(s, x, v):
			psi = self.gen_psi(s)
			f = (np.dot(self.w, psi[:, 0])) / (np.sum(psi[:, 0])) * self.cs.s
			f = np.nan_to_num(np.dot(M,f) + np.array([0.0, 0.0 ,0.0])) 
		...
		# print("Ddx: ", self.ddx,", Dx: ",self.dx, ",x: ",self.x)
