...
def invKQuat(target,quat,calcPivots=False):
	rot=R.from_quat(quat)
	return invKGeneric(target[0:3],rot,calcPivots)

def invK(target,calcPivots=False):
	Rz=rotZ(target[5])
	Ry=rotY(target[4])
	Rx=rotX(target[3])
	rTarget=np.matmul(Rz,np.matmul(Ry,Rx))
	rot= R.from_matrix(rTarget[0:3,0:3])
	return invKGeneric(target[0:3],rot,calcPivots)

#takes in units of [m,m,m,R]
#					x,y,z,Rotation
...
def invKGeneric(target,rot,calcPivots=False):
	...
	rTarget=np.zeros((4,4))
	rTarget[3,3]=1
	rTarget[0:3,0:3]=rot.as_matrix()
	qu=rot.as_quat()
	...
	if valid==0 or cAng1<dangerDeg or cAng2<dangerDeg or cAng3<dangerDeg:
		valid=0
	if calcPivots:
		dzArm=np.subtract([rUTop[0][2][3],rUTop[1][2][3],rUTop[2][2][3]],\
		[rB[0][2][3],rB[1][2][3],rB[2][2][3]])
		print(distToAct)
		print(dzArm)
		pivotJoints=np.arccos([dzArm[0]/distToAct[0][0],dzArm[1]/distToAct[1][0],dzArm[2]/distToAct[2][0]])
		#print(pivotJoints)
		return pivotJoints
	return (joints,valid,qu)
...
# 				[0,1,0],
# 				[0,0,1]])
# print(invKGeneric(target[0:3],rot, True))
