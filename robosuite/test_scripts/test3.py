...
while t<t_final:
	#rotary
	# sim.data.body_xpos.flags.writeable = True
	# sim.model.body_pos.flags.writeable = True
	...
	# mujoco_arena1 = BinsArena(bin1_pos=(2,2,0))
	# mujoco_arena = EmptyArena()
	# ipdb.set_trace()
	# world1 = MujocoWorldBase()
	# mujoco_arena1.set_origin([0.8, 0, 0])
	# world1.merge(mujoco_arena1)
	# model = world1.get_model(mode="mujoco_py")

	# sim = MjSim(model)
	# viewer = MjViewer(sim)
	# mujoco_arena.set_origin([2.8, 5, 0])
	# world.merge(mujoco_arena)
	# model = world.get_model(mode="mujoco_py")
	print("sim.data.get_body_xpos", sim.data.get_body_xpos('Conveyor_belt1'))
	# print(sim.data.get_joint_qvel('sphere_joint0'))
	# sim.data.set_body_xpos('Conveyor_belt1', np.array([1.8, 1, 0.4]))
	sim.model.geom_pos[sim.model.geom_name2id("Conveyor_belt_collision1")] = ([0.8 + np.random.uniform(-1,1), 0, 0.4])
	sim.model.geom_pos[sim.model.geom_name2id("Conveyor_belt_visual1")] = ([0.8 + np.random.uniform(-1,1), 0, 0.4])
	# sim.model.geom_pos[sim.model.geom_name2id("Conveyor_belt_collision2")] = ([4.8 + np.random.uniform(-1,1), 1, 0.8])
	t=t+1
	# sim.data.ctrl[0]=0
	# sim.data.ctrl[1]=0
	# sim.data.ctrl[2]=0
